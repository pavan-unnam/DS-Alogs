...
public class SelectionSortTest {
    ...
    public void testSortInteger3DArrayWithParameters( Integer[][][] integer3DArray, Integer[][][] result) {
        ...
        MatcherAssert.assertThat(integer, Matchers.arrayContainingInAnyOrder(result));
    }
    ...
    @ParameterizedTest
    @MethodSource("org.algos.sort.params.SortAlgoParamsProvider#provideListIntegerArguments")
    public void testSortIntegerListParameters(List<Integer> integerList, List<Integer>  result) {
        List<Integer> list = null;
        Object integerListObject = sortAlgoManager.manageAlgo("integerList", integerList, isRecursive, isColumnSorting);
        if(integerListObject instanceof List<?>) {
            list = (List<Integer>) integerListObject;
        }
        String dsName = sortAlgoManager.getDataStructure().getDataStructureName();
        String algoName = sortAlgoManager.getAlgoName();
        Assertions.assertEquals(SortAlgoEnum.SELECTION.name(),algoName);
        Assertions.assertEquals(DataStructureEnum.INTEGER_LIST.name(),dsName);
        Assertions.assertEquals(integerList,sortAlgoManager.getDataStructure().getIntList());
        assert list != null;
        Assertions.assertTrue(list.size() == result.size() && list.containsAll(result)
                && result.containsAll(list));
    }

    @ParameterizedTest
    @MethodSource("org.algos.sort.params.SortAlgoParamsProvider#provideStringIntegerArguments")
    public void testSortStringListParameters(List<String> stringList, List<String>  result) {
        List<String> list = null;
        Object stringListObject = sortAlgoManager.manageAlgo("stringList", stringList, isRecursive, isColumnSorting);
        if(stringListObject instanceof List<?>) {
            list = (List<String>) stringListObject;
        }
        String dsName = sortAlgoManager.getDataStructure().getDataStructureName();
        String algoName = sortAlgoManager.getAlgoName();
        Assertions.assertEquals(SortAlgoEnum.SELECTION.name(),algoName);
        Assertions.assertEquals(DataStructureEnum.STRING_LIST.name(),dsName);
        Assertions.assertEquals(stringList,sortAlgoManager.getDataStructure().getStringList());
        assert list != null;
        Assertions.assertTrue(list.size() == result.size() && list.containsAll(result)
                && result.containsAll(list));
    }
